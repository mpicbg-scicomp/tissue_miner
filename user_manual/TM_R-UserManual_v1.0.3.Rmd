---
title: "TissueMiner User Manual for TM_1.0.3"
...
#### Define ROI's:
* launch FIJI
* get the the fiji ***draw_n_get_ROIcoord.ijm*** macro from [github](https://raw.githubusercontent.com/mpicbg-scicomp/tissue_miner/master/fiji_macros/draw_n_get_ROIcoord.ijm)
...
#### Orient the tissue
* launch FIJI
* get the the fiji ***orient_tissue.ijm*** macro from [github](https://raw.githubusercontent.com/mpicbg-scicomp/tissue_miner/master/fiji_macros/orient_tissue.ijm)
...
### How to run the "snakemake" automated workflow: select specific rules
* A graph represents the order and the relations between rules (tasks) to be run. You can see such a graph in Figure 7 of the [resource paper](https://elifesciences.org/content/5/e14334).
...
* To make this alias permanent, please copy it into the .bashrc file
* To **limit the resources allocated to the snakemake workflow**, you can set the `--memory` and `--cpuset-cpus` options in the alias above, see [Appendix 2](#appendix-2-rstudio-server-container-configuration) 
...
## How to set up TissueMiner for running API

### Start and access RStudio within TissueMiner 

Here, we describe the API requirements on the Docker plateform.

***

**On any operating systems with the Docker plateform**, you can use **Rstudio** from the provided docker image that includes all necessary R packages and dependencies. 


* **Step1: Create a RStudio container**

Using **Linux**, open a terminal; using **MacOS** and **Windows**, open a Docker Quick Start terminal. Then type in  

```{bash rocker, warning=FALSE, message=FALSE, echo=T, eval=F}
docker run --name rocker -d -p 8787:8787 -e USER=rstudio -e PASSWORD=rstudio \
-v $HOME:/home/rstudio/data \
etournay/tissue_miner
```

Explanations regarding this command:

1. `docker run`: runs "etournay/tissue_miner" in a container
2. `--name rocker`: names the container "rocker"
3. `-d`: runs in the background
4. `-p 8787:8787`: opens ports in the container for access by your webbrowser
5. `-e USER=rstudio -e PASSWORD=rstudio`: defines login and password to connect to the RStudio server from your webbrowser
6. `-v $HOME:/home/rstudio/data`: mounts your home folder inside the container so that you data are visible from the container
7. `etournay/tissue_miner`: the name of the application (docker image) to be run as a docker container

Note: "container" is the term used for an application and its dependencies embedded within the Docker plateform. The "rocker" container is not only created by the above command line, it is also up and running. 

**Important note for Linux users:** still one limitation due to the current Docker version, namely the dockerized TissueMiner only works if the user holds an user ID = 1000 that matches the user ID inside the container. So far there isn't any simple solution to this issue. You won't have read/write permissions if your user ID is different from 1000 and TissueMiner won't work correctly. To know your user ID, just type in `id` in a terminal. If you are the only user of your Linux machine, you should have an user ID = 1000. The only workaround is to install TissueMiner with the [installation script](https://github.com/mpicbg-scicomp/tissue_miner/blob/master/installation/local_installation.md).


* **Step2: Connect to the RStudio server with your web browser**
    + On **Linux**: juste type in `localhost:8787` as a URL in your favorite web browser (then bookmark it !)
    + On **MacOS** and **Windows**, first run the command below to get the URL and copy-paste the result into your web browser 
...
* **Step4: enjoy R programming in RStudio !**
    + You can actually compile this TM_R-UserManual that you'll find as Rmarkdown document on [github](https://github.com/mpicbg-scicomp/tissue_miner/blob/gh-pages/user_manual/TM_R-UserManual_v1.0.1.Rmd)

***

### Stop, start, remove the RStudio container

Once the RStudio server is running within a Docker container called "rocker", you shouldn't run any additional one. Instead, you can control the state of the "rocker" container. The container physically stays on your docker plateform until you explicilty delete it. For example, if you restart the computer, this "rocker" container will still be present but stopped, and you would have to manually start it using `docker start rocker`. If you want to mount additional data shares into the "rocker" container, best is to delete it using `docker stop rocker; docker rm rocker` and create a new one with appropriate options (see [Appendices](#appendices)).

* In your terminal (Linux) or Quick Start Terminal (Mac, Win):
    + **stop:** `docker stop rocker`
    + **start:** `docker start rocker`
    + **restart:** `docker restart rocker`
    + **visualize running containers: ** `docker ps`
    + **delete the rocker container:** `docker stop rocker; docker rm rocker`
    
***

### Access your own configuration files from the RStudio-server container
...
--- How to use your own configuration file in the **dockerized RStudio**:
...
* you can transiently overwrite these variables by using a dedicated R command `Sys.setenv()` that you should always add at the begining of your scripts. 

Example:

```{r set up config and script path, eval=FALSE}
# Example: "data" is the mount point to access your data (see above)
Sys.setenv(TM_CONFIG="/home/rstudio/data/my_favorite_config.R")

# For this tutorial we will use the provided configuration file for fly wing analysis
Sys.setenv(TM_CONFIG="/tissue_miner/config/flywing_tm_config.R")

```

***

## My first R script: load the TissueMiner API

### Set up a header for all of your scripts
...
***
### Some of the functions you have just loaded into the computer memory

TissueMiner provides functions to be used in combination with the existing grammars in R for visualizing and quantifying cell dynamics in 2D-living tissues.
...
render_frame(cellshapesWithRoi, 50) + 
  ...
  scale_fill_manual(values=c("whole_tissue"="darkgreen"))
```
...
rm(cellshapes,cellshapesWithRoi,lgRoiSmoothed)
```
...
TissueMiner provides a set of tools to quantify and visualize cell dynamics at different spatial scales.
These tools are all prefixed with **'mqf_'** as they perform **m**ultiple **q**ueries to the pre-processed data obtained with the TissueMiner automated workflow. This library must be load in the R environment using `source(file.path(scriptsDir, "commons/BaseQueryFunctions.R"))` as described above. These tools have common features:
...
render_movie(cellShapesElong, "CellElongationPattern.mp4", list(
  ...
  scale_fill_gradientn(name="elongation",
                       colours=c("black", "blue", "green", "yellow", "red"),
                       limits=c(0,quantile(cellShapesElong$norm, probs = 99.9/100)),
                       na.value = "red")
))
...
## Appendix 1: Virtualbox configuration (Mac and Windows)

The Docker Toolbox first creates a virtual machine called "default" in Virtualbox. If you open Virtualbox, you should see this virtual machine called "default". You can configure this "default" virtual machine in the "Settings" panel of Virtualbox. In particular, you can:

* set up the CPU number
* set up the allocated memory
* set up additional shares such as `D:\` on Windows or `/Volumes` on Mac

## Appendix 2: RStudio-server container configuration
...
Note: on MacOS and Windows systems, some shares are not visible by default from the "rocker" container. These shares must first be added in Virtualbox, see [Appendix 1](#appendix-1-virtualbox-configuration-mac-and-windows).

* Set up a **memory upper limit** for the container using `--memory`

```{bash rocker memory example, warning=FALSE, message=FALSE, echo=T, eval=F}
# We set up the upper limit to 12Gb of RAM (must be less than the RAM of the computer)
docker run --name rocker --memory 12g -d -p 8787:8787 -e USER=rstudio -e PASSWORD=rstudio \
-v $HOME:/home/rstudio/home_share \
etournay/tissue_miner
```


* Set up a **fixed number of CPUs** allocated to the container using `--cpuset-cpus`

```{bash rocker cpu example, warning=FALSE, message=FALSE, echo=T, eval=F}
# We set up 4 CPUs using the CPU identifiers 0 to 3 
docker run --name rocker --cpuset-cpus 0-3 -d -p 8787:8787 -e USER=rstudio -e PASSWORD=rstudio \
-v $HOME:/home/rstudio/home_share \
etournay/tissue_miner
```
