...
interpolate <- function(df, x_colname, y_colnames, interpolationGroup, delta_x, method = "linear"){
        ...
        if (method %in% c("linear", "constant")){
          with(., approx(x, y,
                         xout = seq(min_x[1], max_x[1],
                                    by = delta_x), method = method)) %>% as.df() %>% rename_(.dots=setNames(names(.), c(x_colname, ycol)))
...
mqf_cg_roi_rate_shear <- function(movieDir, rois=c(), interpol_interval_sec="guess", interpol_method="linear", smooth_time_window_size_sec=3600){
  ...
  # Arguments: movieDir = path to movie directory, rois = selected rois (all by default), interpol_interval_sec =  size of new intervals in seconds, smooth_time_window_size_sec = time over which to average
  # Output: a dataframe
  ...
  kernSize <- round(smooth_time_window_size_sec/interpol_interval_sec)
...
#### Extract shear data ####
shearData <- multi_db_query(movieDirs, mqf_cg_roi_rate_shear, "L5", interpol_interval_sec=60, interpol_method="linear", smooth_time_window_size_sec=2700) %>% print_head()
...
ggplot(shearRateSlim, aes(dev_time,xx_rate_hr.ma*100, color=tensor)) +
  geom_line() + 
  xlab("Time [hAPF]")+
