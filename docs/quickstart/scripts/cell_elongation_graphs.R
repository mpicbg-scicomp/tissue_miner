...
if(length(argv) != 2){
  stop("Usage: cell_elongation_graphs.R <movie_db_directory> <output_directory>")
}else{
...
ggsave2(mqf_cg_roi_nematics_cell_elong(movieDir, rois = c("raw")) %>% 
...
ggsave2(mqf_fg_nematics_cell_elong(movieDir, rois = c("raw")) %>% 
