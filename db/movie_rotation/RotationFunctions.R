...
readTrafoFile <- function(fileName){
  ...
  dat$IsHorizontalFlip <- as.logical(dat$IsHorizontalFlip) # NEW
  dat
}
...
applyTrafo <- function(dat, x, y) {
  ...
  if (dat$IsHorizontalFlip) (xTrafo=abs(xTrafo-dat$newImWidth)) # NEW
...
trafoNematic <- function (dat, Txx, Txy){
  ...
  #  Flipping nematics only affects the xy component 
